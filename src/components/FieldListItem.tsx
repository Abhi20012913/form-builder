...
export default function FieldListItem({ field, index, total, onEdit, onDelete, onMoveUp, onMoveDown }: Props) {
  return (
    <ListItem divider>
      <ListItemText
      ...
      />
      <ListItemSecondaryAction>
        <IconButton edge="end" onClick={() => onMoveUp(index)} disabled={index === 0} size="large">
          <ArrowUpward />
        </IconButton>
        <IconButton edge="end" onClick={() => onMoveDown(index)} disabled={index === total - 1} size="large">
          <ArrowDownward />
        </IconButton>
